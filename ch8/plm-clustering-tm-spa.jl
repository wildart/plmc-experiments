...
if length(ARGS) > 1
    ...
    if length(ARGS) > 2
        ...
        if length(ARGS) > 3
            rngdata = parse(Bool, ARGS[4])
            println("Changing data component: ", rngdata)
        end
    end
end
...
εs = [0.1, 0.2, 0.3]
X, L = nothing, nothing
generated = Dict(ε=>false for ε in εs)
for ε in εs, i in startindex:PARAMS[:iterations]
    global X, L, generated
    print("\n$i $ε: ")
    tm = time()
    PARAMS[:ε] = ε

    ## generate on first iteration
    X, L = if !generated[ε] || rngdata
        generated[ε] = true
        PARAMS[:data_seed] += i-1
        nmoons(Float64, PARAMS[:m], PARAMS[:c], ε=PARAMS[:ε], r=PARAMS[:r], d=PARAMS[:d],
    ...
    RESULTS.TMDL = String => Vector{Float64}
    RESULTS.TERR = String => Vector{Float64}
    RESULTS.TCOMP = String => Vector{Float64}
    RESULTS.TCLUS = String => Vector{Vector{Vector{Int}}}

    if !rngdata # reset seed for cover
        PARAMS[:cover_seed] += 1
    end
    ...
    for cl in CLUSTERING, cv in COVERS
        ...
        if agg !== nothing
            ...
            mthd = "$cl+$cv:top"
            C, mdl, err, comp = try
                MDL, LL, Rₘₐₓ = testmerges(PLMC.nml, agg.clusters, MC, X)
                plmc(agg, MC, X, flt, score=PLMC.nml, find=findscore), MDL, LL, Rₘₐₓ
            catch ex
                print("-ERR")
                nothing, zeros(0), zeros(0), zeros(0)
            end
            RESULTS.TMDL = mthd => mdl
            RESULTS.TERR = mthd => err
            RESULTS.TCOMP = mthd => comp
            RESULTS.TCLUS = mthd => agg.clusters
            RESULTS.NMI, RESULTS.RAND, RESULTS.ASSIGN = saveindecies(mthd, L, C)
        end
        # mdl aglomeration
        print(" MDL")
        C = try
    ...
    # Save results
    if i % 100 == 0
        fname = "plm-clustering-tm$(PARAMS[:c])-d$(PARAMS[:d])-spa"
        fname *= rngdata ? "-data" : "-cover"
